<div class="row">
  <div class="col-md-8">
    ...
    <div class="form-group position-relative">
      ...
      <input type="hidden" id="customer_phone_no" name="customer_phone_no"
        value="{{ old('customer_phone_no', isset($insuranceAdvance->customer) ? $insuranceAdvance->customer->phone_no : '') }}">
      ...
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" id="customer_name" name="customer_name" class="form-control" readonly
            value="{{ old('customer_name', isset($insuranceAdvance->customer) ? $insuranceAdvance->customer->name : '') }}">
        </div>
      </div>
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" id="customer_pan_no" name="customer_pan_no" class="form-control" readonly
            value="{{ old('customer_pan_no', isset($insuranceAdvance->customer) ? $insuranceAdvance->customer->pan_number : '') }}">
        </div>
      </div>
    </div>
      ...
      <div class="col-md-12">
        <div class="form-group">
          ...
          <input type="text" name="proposal_policy_number"
            ...
            value="{{ old('proposal_policy_number', $insuranceAdvance->proposal_policy_number ?? '') }}" required>
    ...
    <div class="form-group">
      ...
      <input type="text" name="so_name" class="form-control @error('so_name') is-invalid @enderror" id="so_name"
        placeholder="Enter So Name" value="{{ old('so_name', $insuranceAdvance->so_name ?? '') }}" required>
      @error('so_name')
      <small class="error invalid-feedback" role="alert">{{ $message }}</small>
      @enderror
    </div>
    ...
    <div class="form-group">
      ...
      <input type="number" step="0.01" name="total_amount"
        ...
        placeholder="Enter total amount" value="{{ old('total_amount', $insuranceAdvance->total_amount ?? '') }}"
        required>
      @error('total_amount')
      <small class="error invalid-feedback" role="alert">{{ $message }}</small>
      @enderror
    ...
    <div class="form-group d-flex justify-content-between mt-4">
      <div>
        <button type="submit" class="btn btn-primary" name="action" value="save">
          {{ isset($insuranceAdvance) ? 'Update' : 'Save' }}
        </button>
        <button type="submit" class="btn btn-info" name="action" value="save_generate_receipt">
          {{ isset($insuranceAdvance) ? 'Update and Generate Receipt' : 'Save and Generate Receipt' }}
        </button>
      </div>
      <a href="{{ route('admin.insurance-advances.index') }}" class="btn btn-secondary">Cancel</a>
    </div>
  </div>
  ...
  <div class="col-md-4">
    ...
    <div class="text-left">
      @if(isset($insuranceAdvance) && $insuranceAdvance->id)
      <a href="{{ route('admin.insurance-advances.receipt', $insuranceAdvance) }}" class="btn btn-info" target="_blank">
...
@include('admin.insurance-advances._scripts')
