<div class="row">
  <div class="col-md-8">
    ...
    <div class="form-group position-relative">
      ...
      <input type="hidden" id="customer_phone_no" name="customer_phone_no"
        value="{{ old('customer_phone_no', isset($vasInvoice->customer) ? $vasInvoice->customer->phone_no : '') }}">
      ...
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" id="customer_name" name="customer_name" class="form-control" readonly
            value="{{ old('customer_name', isset($vasInvoice->customer) ? $vasInvoice->customer->name : '') }}">
        </div>
      </div>
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" id="customer_pan_no" name="customer_pan_no" class="form-control" readonly
            value="{{ old('customer_pan_no', isset($vasInvoice->customer) ? $vasInvoice->customer->pan_number : '') }}">
        </div>
      </div>
    </div>

    <!-- Booking Number and Invoice Number in one row -->
    <div class="row">
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" name="order_booking_number"
            ...
            value="{{ old('order_booking_number', $vasInvoice->order_booking_number ?? '') }}" required>
      ...
      <div class="col-md-6">
        <div class="form-group">
          ...
          <input type="text" name="invoice_number" class="form-control @error('invoice_number') is-invalid @enderror"
            ...
            value="{{ old('invoice_number', $vasInvoice->invoice_number ?? '') }}" required>
    ...
    <div class="form-group">
      ...
      <input type="text" name="so_name" class="form-control @error('so_name') is-invalid @enderror" id="so_name"
        placeholder="Enter So Name" value="{{ old('so_name', $vasInvoice->so_name ?? '') }}" required>
      @error('so_name')
      <small class="error invalid-feedback" role="alert">{{ $message }}</small>
      @enderror
    </div>
    ...
    <div class="form-group">
      ...
      <input type="number" step="0.01" name="total_amount"
        ...
        placeholder="Enter total amount" value="{{ old('total_amount', $vasInvoice->total_amount ?? '') }}" required>
      @error('total_amount')
      <small class="error invalid-feedback" role="alert">{{ $message }}</small>
      @enderror
    </div>
    ...
    <div class="form-group d-flex justify-content-between mt-4">
      <div>
        <button type="submit" class="btn btn-primary" name="action" value="save">
          {{ isset($vasInvoice) ? 'Update' : 'Save' }}
        </button>
        <button type="submit" class="btn btn-info" name="action" value="save_generate_receipt">
          {{ isset($vasInvoice) ? 'Update and Generate Receipt' : 'Save and Generate Receipt' }}
        </button>
      </div>
      <a href="{{ route('admin.vas-invoices.index') }}" class="btn btn-secondary">Cancel</a>
    </div>
  </div>
  ...
  <div class="col-md-4">
    ...
    <div class="text-left">
      @if(isset($vasInvoice) && $vasInvoice->id)
      <a href="{{ route('admin.vas-invoices.receipt', $vasInvoice) }}" class="btn btn-info" target="_blank">
...
@include('admin.vas-invoices._scripts')
